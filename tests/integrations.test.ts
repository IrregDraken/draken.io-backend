...
describe('integration boundaries', () => {
  ...
  it('keeps Telegram notifications behind the NotificationService contract', async () => {
    ...
    const client = {
      isConfigured: () => true,
      sendMessage: async (_recipient: number | string, body: string) => {
        sent.push(body);
      },
    ...
    expect(
      (await notifications.send({ recipient: 'not-a-chat-id', body: 'hello' })).delivered,
    ).toBe(false);
  ...
  it('uses a real OpenAI adapter behind the provider interface', async () => {
    ...
    const registry = new AIProviderRegistry(
      loadConfig({ NODE_ENV: 'test', OPENAI_API_KEY: 'present-for-test' }),
    );
    const provider = registry.get('openai');
    expect(provider.name).toBe('openai');
    expect(provider.isConfigured()).toBe(true);
    expect((await provider.healthCheck()).status).toBe('ok');
    await expect(provider.generate({ model: 'gpt-test', prompt: 'hello' })).resolves.toMatchObject({
