...
export interface ToolExecutionRepository {
  createToolExecution(input: ToolExecutionRequest): Promise<string>;
  listTools(companyId: string): Promise<ToolDefinition[]>;
  hasEmployeeTool(companyId: string, employeeId: string, toolId: string): Promise<boolean>;
  finishToolExecution(
    companyId: string,
    executionId: string,
...
export class ToolExecutionService {
  ...
  async execute(input: ToolExecutionRequest): Promise<{
    executionId: string;
    status: string;
    ...
    try {
      ...
      if (!input.employeeId) {
        await this.repository.finishToolExecution(input.companyId, executionId, {
          status: 'denied',
          error: 'Tool execution requires an agent identity',
        });
        return {
          executionId,
          status: 'denied',
      ...
      if (
        !(await this.repository.hasEmployeeTool(input.companyId, input.employeeId, input.toolId))
      ) {
        await this.repository.finishToolExecution(input.companyId, executionId, {
          status: 'denied',
      ...
      await this.repository.writeActivity({
        companyId: input.companyId,
        actorEmployeeId: input.employeeId,
        activityType: 'tool.succeeded',
        taskId: input.taskId,
        toolExecutionId: executionId,
        message: `Tool ${definition.name} completed`,
        metadata: { toolId: definition.id },
      });
      return { executionId, status: 'succeeded', output };
    } catch (error) {
      const message = error instanceof Error ? error.message : 'Tool execution failed';
      this.logger.error(
        { toolId: input.toolId, executionId, error: message },
      ...
      await this.repository.writeActivity({
        ...
        taskId: input.taskId,
        toolExecutionId: executionId,
        message: `Tool ${definition?.name ?? input.toolId} failed`,
...
function validateInput(schema: unknown, input: Record<string, unknown>): void {
  ...
  if (jsonSchema.type && jsonSchema.type !== 'object')
    throw new Error('Tool input schema must describe an object');
  const required = Array.isArray(jsonSchema.required)
    ? jsonSchema.required.filter((value): value is string => typeof value === 'string')
    : [];
  for (const key of required)
    if (!(key in input)) throw new Error(`Missing required tool input: ${key}`);
