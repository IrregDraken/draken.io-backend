import type { AIProvider, AIProviderName } from '../integrations/ai.js';
import type { Mission } from '../domain.js';

export interface AIProviderLookup {
...
export class CommandService {
  constructor(
    ...
    private readonly repository: MissionWriter,
  ) {}
  ...
  async execute(input: {
    ...
    const response = await this.providers.get(input.provider).generate({
      ...
      systemInstructions:
        'You are the Draken mission planner. Return only valid JSON. Do not claim tasks, agents, tools, or outputs that were not provided.',
    });
    ...
    return {
      ...
      execution: 'mission_created_unassigned' as const,
    };
  }
}
...
function parsePlan(raw: string): {
  ...
  try {
    ...
    throw new Error('AI provider did not return valid mission JSON');
  }
  if (!parsed || typeof parsed !== 'object')
    throw new Error('AI provider returned an invalid mission plan');
  ...
  if (!title || !objective || ![1, 2, 3, 4, 5].includes(priority as number))
    throw new Error('AI provider returned an incomplete mission plan');
