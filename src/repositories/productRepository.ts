...
export class ProductRepository {
  ...
  async listMissions(companyId: string): Promise<Mission[]> {
    ...
    const { data, error } = await client
      .from('missions')
      .select('*, mission_agents(employee_id), tasks(id), mission_outputs(id)')
      .eq('company_id', companyId)
    ...
    return (data ?? []).map((row: Row) => ({
      ...
      deadline: optionalString(row.due_at),
      status: String(row.status),
      stage: String(row.stage ?? 'created') as MissionStage,
      progress: Number(row.progress ?? 0),
      assignedAgentIds: rows(row.mission_agents).map((agent) => String(agent.employee_id)),
      taskCount: rows(row.tasks).length,
      outputCount: rows(row.mission_outputs).length,
  ...
  async createMission(input: {
    ...
    if (input.assignedAgentIds.length > 0) {
      const { error: assignmentError } = await client.from('mission_agents').insert(
        input.assignedAgentIds.map((employeeId) => ({
          mission_id: data.id,
    ...
    return {
      ...
      deadline: data.due_at ?? undefined,
      status: String(data.status),
      stage: String(data.stage) as MissionStage,
  ...
  async transitionMission(input: {
    ...
    const client = requireAdmin(this.admin);
    const { data: mission, error: findError } = await client
      .from('missions')
    ...
    if (!isValidMissionTransition(current, input.stage))
      throw new Error(`Invalid mission transition from ${current} to ${input.stage}`);
    const status =
      input.stage === 'completed'
        ? 'completed'
        : input.stage === 'failed'
          ? 'failed'
          : input.stage === 'executing'
            ? 'active'
            : input.stage === 'review'
              ? 'active'
              : 'planned';
    ...
    await this.recordEvent({
      companyId: input.companyId,
      actorUserId: input.actorUserId,
      eventType: `mission.${input.stage}`,
      aggregateType: 'mission',
      aggregateId: input.missionId,
      payload: { from: current, to: input.stage, failureReason: input.failureReason },
    });
  }
  ...
  async createTask(input: {
    ...
    const client = requireAdmin(this.admin);
    await this.ensureOptionalForeignKeys(
      input.companyId,
      input.missionId,
      input.projectId,
    ...
    await this.recordEvent({
      ...
      aggregateType: 'task',
      aggregateId: String(data.id),
      payload: { missionId: input.missionId, title: input.title },
    });
  ...
  async transitionTask(input: {
    ...
    if (!isValidTaskTransition(current, input.status))
      throw new Error(`Invalid task transition from ${current} to ${input.status}`);
    const timestamp = new Date().toISOString();
    const patch = {
      status: input.status,
      failure_reason: input.failureReason ?? null,
      blocked_reason:
        input.blockedReason ?? (input.status === 'blocked' ? existing.blocked_reason : null),
      output: input.output ?? existing.output ?? null,
      ...(input.status === 'in_progress' ? { started_at: timestamp } : {}),
      ...(input.status === 'done' || input.status === 'failed' ? { completed_at: timestamp } : {}),
    };
    const { data, error } = await client
      .from('tasks')
      .update(patch)
    ...
    await this.recordEvent({
      ...
      payload: { from: current, to: input.status, failureReason: input.failureReason },
    });
    await this.writeActivity({
      companyId: input.companyId,
      actorUserId: input.actorUserId,
      activityType: `task.${input.status}`,
      taskId: input.taskId,
      message: `Task transitioned from ${current} to ${input.status}`,
  ...
  async retryTask(input: {
    ...
    await this.recordEvent({
      ...
      aggregateType: 'task',
      aggregateId: input.taskId,
      payload: { retryCount: retryCount + 1, retryLimit },
    });
    return this.task(data);
  }
  ...
  async addDependency(input: {
    ...
    if ((tasks ?? []).length !== 2)
      throw new Error('Task dependency crosses company boundary or references a missing task');
    const { error } = await client
      .from('task_dependencies')
      .insert({ task_id: input.taskId, depends_on_task_id: input.dependsOnTaskId });
    if (error) throw error;
  }
  ...
  async dependenciesComplete(companyId: string, taskId: string): Promise<boolean> {
    ...
    return (
      ...
      (tasks ?? []).every((task: Row) => task.status === 'done')
    );
  }
  ...
  async hasEmployeeTool(companyId: string, employeeId: string, toolId: string): Promise<boolean> {
    const { data: employee, error: employeeError } = await requireAdmin(this.admin)
      .from('employees')
      .select('id')
      .eq('company_id', companyId)
      .eq('id', employeeId)
    ...
    const { data: tool, error: toolError } = await requireAdmin(this.admin)
      .from('tools')
      .select('id')
      .eq('company_id', companyId)
      .eq('id', toolId)
  ...
  async createToolExecution(input: ToolExecutionRequest): Promise<string> {
    const client = requireAdmin(this.admin);
    const { data: tool, error: toolError } = await client
      .from('tools')
      .select('id')
      .eq('company_id', input.companyId)
      .eq('id', input.toolId)
  ...
  async finishToolExecution(
    companyId: string,
    executionId: string,
    result: { status: 'succeeded' | 'failed' | 'denied'; output?: unknown; error?: string },
  ): Promise<void> {
  ...
  async claimPendingBusEvents(limit = 20): Promise<
    Array<{
      id: string;
      companyId?: string;
      eventType: string;
    ...
    for (const row of (data ?? []) as Row[]) {
      ...
      if (updated)
        claimed.push({
          ...
          eventType: String(updated.event_type),
          aggregateType: updated.aggregate_type ?? undefined,
          aggregateId: updated.aggregate_id ?? undefined,
  ...
  async finishBusEvent(
    id: string,
    result: { status: 'processed' | 'failed'; error?: string },
  ): Promise<void> {
    const { error } = await requireAdmin(this.admin)
      .from('message_bus_events')
      .update({
  ...
  async recordEvent(input: {
    ...
    const event = {
      ...
      actor_user_id: input.actorUserId ?? null,
      event_type: input.eventType,
      entity_type: input.aggregateType ?? null,
      entity_id: input.aggregateId ?? null,
      payload: input.payload ?? {},
    };
    const { error } = await client.from('events').insert(event);
    if (error) throw error;
    const { error: busError } = await client.from('message_bus_events').insert({
      company_id: input.companyId,
      event_type: input.eventType,
      aggregate_type: input.aggregateType ?? null,
      aggregate_id: input.aggregateId ?? null,
  ...
  private async ensureEmployeesInCompany(companyId: string, employeeIds: string[]): Promise<void> {
    ...
    const { data, error } = await requireAdmin(this.admin)
      ...
      .in('id', employeeIds);
    if (error) throw error;
    if ((data ?? []).length !== employeeIds.length)
      throw new Error('One or more employees do not belong to the requested company');
  }

  private async ensureOptionalForeignKeys(
    companyId: string,
    missionId?: string,
    projectId?: string,
    assigneeEmployeeId?: string,
    ...
    if (missionId) {
      const { data, error } = await client
        ...
        .maybeSingle();
      if (error) throw error;
      if (!data) throw new Error('Mission does not belong to the requested company');
    }
    if (projectId) {
      const { data, error } = await client
        .from('projects')
        ...
        .maybeSingle();
      if (error) throw error;
      if (!data) throw new Error('Project does not belong to the requested company');
    }
  ...
  private employee(row: Row): Employee {
    return {
      ...
      tools: row.permissions ?? {},
      status: String(row.status),
      currentMissionId: optionalString(row.current_mission_id),
      currentTaskId: optionalString(row.current_task_id),
      currentAssignment: optionalString(row.current_assignment),
    };
  }
  ...
  private task(row: Row): Task {
    return {
      ...
      assigneeEmployeeId: optionalString(row.assignee_employee_id),
...
const missionTransitions: Record<MissionStage, MissionStage[]> = {
  created: ['planning', 'failed'],
  planning: ['executing', 'failed'],
  executing: ['review', 'failed'],
  review: ['completed', 'executing', 'failed'],
  completed: [],
  failed: ['planning'],
};
function isValidMissionTransition(current: MissionStage, next: MissionStage): boolean {
  return current === next || missionTransitions[current]?.includes(next) === true;
}

const taskTransitions: Record<string, string[]> = {
  backlog: ['todo', 'cancelled'],
  todo: ['in_progress', 'blocked', 'cancelled'],
  in_progress: ['blocked', 'review', 'done', 'failed', 'cancelled'],
...
function isValidTaskTransition(current: string, next: string): boolean {
  return taskTransitions[current]?.includes(next) === true;
}
