...
export type { AIProvider, AIProviderName } from '../domain.js';
...
abstract class HttpAIProvider implements AIProvider {
  ...
  async healthCheck(): Promise<ComponentHealth> {
    if (!this.apiKey)
      return { status: 'unconfigured', detail: `${this.name} credentials are not configured` };
    try {
      ...
      return {
        status: 'error',
        detail: error instanceof Error ? error.message : `${this.name} health check failed`,
      };
    }
  }
...
class OpenAIProvider extends HttpAIProvider {
  ...
  protected async checkRemote(): Promise<void> {
    await requestJson(this.url('/models'), {
...
class AnthropicProvider extends HttpAIProvider {
  ...
  protected async checkRemote(): Promise<void> {
    await requestJson('https://api.anthropic.com/v1/models', {
      headers: { 'x-api-key': this.requireKey(), 'anthropic-version': '2023-06-01' },
    });
  }
  async generate(request: AIProviderRequest): Promise<AIProviderResponse> {
    const result = await requestJson<{ content?: Array<{ type?: string; text?: string }> }>(
...
class GeminiProvider extends HttpAIProvider {
  ...
  protected async checkRemote(): Promise<void> {
    await requestJson(
      `https://generativelanguage.googleapis.com/v1beta/models?key=${encodeURIComponent(this.requireKey())}`,
    );
  }
  async generate(request: AIProviderRequest): Promise<AIProviderResponse> {
    const result = await requestJson<{
      ...
      `https://generativelanguage.googleapis.com/v1beta/models/${encodeURIComponent(request.model)}:generateContent?key=${encodeURIComponent(this.requireKey())}`,
      {
        method: 'POST',
        body: {
  ...
  async healthCheck(): Promise<ComponentHealth> {
    return {
      ...
      detail: this.configuredCredential
        ? `${this.name} credentials are present but no stable adapter contract is configured`
